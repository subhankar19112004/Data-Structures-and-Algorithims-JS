...
var lastStoneWeight = function (stones) {
  ...
  let n = this.heap.size();
  for (let i = 0; i <= n; i++) {
    let firstMax = this.heap.dequeue();
    let secondMax = this.heap.dequeue();
    if (firstMax && secondMax) {
      if (firstMax == secondMax) continue;
      let result = firstMax - secondMax;
      this.heap.enqueue(result);
    } else {
      this.heap.enqueue(firstMax);
    }
  }
  // let firstMax = this.heap.dequeue();
  // let secondMax = this.heap.dequeue();
